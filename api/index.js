...
function getApp() {
  if (!app) {
    ...
    const authenticate = async (req, res, next) => {
      ...
      // Method 4: Try custom session token format: 'eyJ' + base64(JSON payload)
      // This is the format generated by the /api/auth/login Tookan login endpoint
      if (!user) {
        try {
          let tokenBody = token;
          // Strip the fake JWT prefix 'eyJ' if present
          ...
          debugErrors.push(`Custom session token decode exception: ${e.message}`);
        }
      }
