  ...
  if (!app) {
    ...
    app.post('/api/webhooks/tookan/customer', async (req, res) => {
      try {
        ...
        if (expected && bodySecret !== expected) {
          console.log('âŒ Webhook secret mismatch. Expected:', expected, 'Got:', bodySecret);
        ...
        if (!customerId) {
          console.log('âŒ No customer ID found in payload');
          return res.status(400).json({ status: 'error', message: 'vendor_id is required' });
        }

        if (!isSupabaseConfigured || !supabase) {
          console.log('âŒ Supabase not configured');
        ...
        console.log(`âœ… Customer ${customerId} synced via webhook (Vercel)`);
    ...
    app.get('/api/tookan/orders/cached', async (req, res) => {
      try {
        ...
        console.log(`ðŸ” DEBUG API: Filtered Total: ${total} (Raw: ${allLightData?.length}). Page: ${pageNum}`);
...
app.post('/api/tookan/order/reorder', authenticate, requirePermission(PERMISSIONS.PERFORM_REORDER), async (req, res) => {
  try {
    ...
    const orderData = {
      ...
      codAmount: codAmount !== undefined ? codAmount : (parseFloat(original.cod) || 0),
      orderFees: orderFees !== undefined ? orderFees : (parseFloat(original.order_payment) || 0),
      assignedDriver: assignedDriver !== undefined ? assignedDriver : (original.fleet_id || null),
    ...
    const data = await response.json();
    const newOrderId = data.data?.job_id || null;

    // Save to Supabase
    if (newOrderId && isSupabaseConfigured && supabase) {
      try {
        ...
        console.error('Failed to save reorder to Supabase:', dbErr.message);
      }
    }

    res.json({
      status: data.status === 200 ? 'success' : 'error',
      message: data.message || 'Re-order created successfully',
      data: { newOrderId, originalOrderId: orderIdToUse }
    });
  } catch (error) {
    res.status(500).json({ status: 'error', message: error.message });
  }
...
app.get('/api/reports/summary', authenticate, async (req, res) => {
  try {
    ...
    console.log(`ðŸš€ [VERCEL-BACKEND] Summary: customers=${totals.customers}`);
...
app.post('/api/webhooks/tookan/task', async (req, res) => {
  try {
    ...
    if (!expected || (secretHeader !== expected && bodySecret !== expected)) {
      console.warn('âš ï¸  Vercel Webhook: Unauthorized attempt');
      return res.status(401).json({ status: 'error', message: 'Unauthorized' });
    }
    ...
    if (isDeleted) {
      ...
      console.log('âœ… Vercel Webhook: Deleted task removed:', jobId);
    ...
    console.log('ðŸ“¥ Vercel Webhook payload keys:', Object.keys(payload));
    ...
    console.log('â³ Vercel Webhook: Waiting 10s for data propagation...');
    await new Promise(resolve => setTimeout(resolve, 10000));

    let freshTask = null;
    try {
      console.log(`ðŸ”„ Vercel Webhook: Fetching fresh details for Job ID: ${jobId}`);
      const apiKey = getApiKey();
      ...
      if (getResponse.ok) {
        ...
        if (getData.status === 200 && getData.data) {
          ...
          console.log('âœ… Vercel Webhook: Fresh task details fetched');
        }
      }
    } catch (fetchError) {
      console.error('âš ï¸ Vercel Webhook: Failed to fetch fresh details:', fetchError.message);
    }
    ...
    try {
      console.log(`ðŸ’° Vercel Webhook: Fetching COD amount for Job ID: ${jobId}`);
      const apiKey = getApiKey();
          ...
          if (Array.isArray(customFields)) {
            ...
            if (codField && codField.data) {
              ...
              if (!isNaN(codValue)) {
                ...
                console.log('âœ… Vercel Webhook: COD amount found:', codValue);
              }
            }
          }
      ...
      console.error('âš ï¸ Vercel Webhook: Failed to fetch COD amount:', codFetchError.message);
    }
    ...
    console.log('ðŸ“ Vercel Record completed_datetime value:', record.completed_datetime);

    const { error } = await supabase.from('tasks').upsert(record, { onConflict: 'job_id' });
    if (error) throw error;

    console.log('âœ… Vercel Webhook: Task upserted successfully:', jobId);
    res.json({ status: 'success', message: 'Task upserted' });
  } catch (err) {
    console.error('âŒ Vercel Webhook error:', err.message);
...
app.put('/api/tookan/order/:orderId', authenticate, requirePermission('edit_order_financials'), async (req, res) => {
  ...
  try {
    ...
    if (tookanSuccess) {
      log('âœ… Tookan API update successful');
    } else {
      log('âš ï¸ Tookan update failed:', tookanData.message || textResponse);
    }
    ...
    if (supabase) {
      try {
        ...
        log('âš ï¸ Database update failed:', dbError.message);
      }
    } else {
      log('âš ï¸ Skipping database update: Client not available');
    }
