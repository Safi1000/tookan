...
app.get('/api/plans/customer-counts', authenticate, async (req, res) => {
  try {
    ...
    // Get all merchants with plan_id
...
app.get('/api/customers/assigned', authenticate, async (req, res) => {
  try {
    ...
    // Fetch merchants with plan_id set
    const { data: customers, error } = await supabase
      .from('merchants')
...
app.get('/api/customers/with-plans', authenticate, async (req, res) => {
  try {
    ...
    // Map merchants with plan details
    const result = customers.map(c => ({
...
// GET single customer plan by merchant_id (external API with API key auth)
app.get('/api/get_customer_plan/:merchant_id', validateApiKey, async (req, res) => {
  try {
    ...
    // If merchant has no plan, return null plan
    if (!customer.plan_id) {
      return res.json({
        status: 'success',
        data: {
...
app.get('/api/get_all_customers_with_withdraw_fees', validateApiKey, async (req, res) => {
  try {
    ...
    if (error) {
      console.error('Get merchants with withdraw fees error:', error);
      return res.status(500).json({
        status: 'error',
...
// GET withdrawals by vendor_id (external API with API key auth)
// Returns all withdrawal records for a specific vendor with their statuses
app.get('/api/get_vendor_withdrawals/:vendor_id', validateApiKey, async (req, res) => {
  try {
    if (!isConfigured()) {
      return res.status(500).json({ status: 'error', message: 'Database not configured' });
    }
...
// Set global withdrawal fee for all merchants
app.post('/api/withdrawal-fees/set', authenticate, async (req, res) => {
  try {
    ...
    if (isConfigured()) {
      ...
      if (error) {
        console.error('Update all merchants fee error:', error);
      }
    }

    res.json({
      status: 'success',
      message: 'Global withdrawal fee updated for all merchants',
...
app.post('/api/merchant/wallet/batch-balances', authenticate, async (req, res) => {
  try {
    ...
    // Build a map of vendor_id -> wallet_balance
    const balances = {};
...
app.post('/api/tookan/order/return', authenticate, requirePermission('panel_order_editor'), async (req, res) => {
  try {
    ...
    // Also fetch the connected task (via pickup_delivery_relationship) for more reliable coordinates
    let originalCoordinatesFound = false;
    let originalPickup = null;
    ...
    if (isConfigured()) {
      try {
        ...
        if (!dbError && dbTask) {
          ...
          originalPickup = dbTask;
          originalDelivery = dbTask;
          ...
          if (relationshipId) {
            try {
              ...
              console.error('⚠️ Failed to fetch related tasks from DB:', relErr.message);
            }
          }
          ...
          if (orderData.job_latitude && orderData.job_longitude) {
            ...
            console.log('  Pickup coords (merchant):', orderData.job_pickup_latitude, orderData.job_pickup_longitude);
            console.log('  Delivery coords (customer):', orderData.job_latitude, orderData.job_longitude);
          }
        }
      } catch (err) {
    ...
    // Extract pickup_name/delivery_name from the DELIVERY task of the pair
    const dbPickupName = (originalDelivery && originalDelivery.pickup_name) || '';
    const dbDeliveryName = (originalDelivery && originalDelivery.delivery_name) || '';
    console.log('DB pickup_name/delivery_name from delivery task (for return):', { dbPickupName, dbDeliveryName });
    ...
    const effectivePickupName = deliveryName || dbDeliveryName || orderData.customerName || 'Customer';
    const effectiveDeliveryName = pickupName || dbPickupName || orderData.customerName || 'Customer';
    ...
    if (tags && tags.length > 0) {
      combinedPayload.tags = tags;
    }

    console.log('Creating combined PICKUP + DELIVERY task for return order (single API call)...');
    console.log('Combined return payload:', JSON.stringify({ ...combinedPayload, api_key: '***HIDDEN***' }, null, 2));
    ...
    const createTextResponse = await createResponse.text();
    let createData;
    try {
      createData = JSON.parse(createTextResponse);
    } catch (parseError) {
      return res.status(500).json({
        status: 'error',
        message: `API returned non-JSON response: ${createTextResponse.substring(0, 200)}`,
        data: {}
      });
    }
    ...
    console.log('âœ… Combined task created - Pickup:', pickupOrderId, 'Delivery:', deliveryOrderId);
    console.log('âœ… Return order complete (single API call) - Pickup:', pickupOrderId, 'Delivery:', deliveryOrderId);
  ...
  try {
    ...
    const orders = (result.tasks || []).map(task => {
      ...
      return {
        ...
        pickupName: task.pickup_name || '',
        delivery_name: task.delivery_name || '',
        deliveryName: task.delivery_name || '',
...
app.get('/api/tookan/job/:jobId/related-address', authenticate, async (req, res) => {
  try {
    ...
    if (oppositeTask) {
      ...
      // Fetch pickup_name and delivery_name from the DELIVERY task in DB
      // The delivery task is job_type === 1 (or the opposite task if current is pickup)
      let pickupName = '';
      let deliveryName = '';
      const deliveryTask = relatedTasks.find(t => t.job_type === 1) || oppositeTask;
      const deliveryJobId = deliveryTask.job_id;

      if (isConfigured() && deliveryJobId) {
        try {
          const { data: dbTask, error: dbErr } = await supabase
            .from('tasks')
            .select('pickup_name, delivery_name')
            .eq('job_id', deliveryJobId)
            .single();

          if (!dbErr && dbTask) {
            pickupName = dbTask.pickup_name || '';
            deliveryName = dbTask.delivery_name || '';
            console.log('Found pickup_name/delivery_name from delivery task DB:', { pickupName, deliveryName });
          }
        } catch (dbLookupErr) {
          console.warn('Failed to fetch pickup_name/delivery_name from DB:', dbLookupErr.message);
        }
      }
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    if (isConfigured()) {
      ...
      try {
         adminProfile = await userModel.getUserByEmail(email.toLowerCase());
         if (adminProfile) {
            isAdmin = true;
         }
      } catch (err) {
