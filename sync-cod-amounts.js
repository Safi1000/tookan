...
/**
 * Lightweight COD sync for a single job ID
 * Directly fetches from Tookan and updates DB - optimized for serverless
 */
async function syncCodForJobId(jobId) {
    if (!jobId) {
        console.error('❌ syncCodForJobId: No job ID provided');
        return { success: false, message: 'No job ID provided' };
    }

    console.log(`📥 syncCodForJobId: Syncing COD for job ${jobId}...`);

    try {
        // Directly fetch from Tookan API
        const detailsMap = await fetchJobDetailsForJobIds([parseInt(jobId)]);
        const details = detailsMap[jobId];
        const codAmount = details?.cod_amount;

        if (codAmount === null || codAmount === undefined) {
            console.log(`⚠️ syncCodForJobId: No COD found for job ${jobId}`);
            return { success: true, updated: false, message: 'No COD found' };
        }

        // Update only cod_amount in Supabase
        ...
        if (error) {
            console.error(`❌ syncCodForJobId: Failed to update job ${jobId}:`, error.message);
            return { success: false, message: error.message };
        }

        console.log(`✅ syncCodForJobId: Updated job ${jobId} with COD ${codAmount}`);
        return { success: true, updated: true, codAmount };
    } catch (error) {
        console.error(`❌ syncCodForJobId: Error for job ${jobId}:`, error.message);
        return { success: false, message: error.message };
    }
}

module.exports = { syncCodAmounts, syncCodForJobId };
